...
- **R14 — Following/Search Phase 0 범위(LOCK):** **Following** = current watch 집합(R13)의 전용 클라 화면 — watched currents만, board를 대체하지 않는 보조 렌즈(anti-bubble §2.1). Phase 0 = localStorage(`jetstream.watch`/`.seen`, 상태변화 표시), Phase 1 = `/v1/watch*`. **Search** Phase 0 = currents(name+brief) + grounded timeline events에 대한 Postgres **ILIKE**(임베딩 미사용); `SearchHit{type∈{current,event}, …}`는 §7.3 SearchHit의 부분집합(상위호환). Phase 1에서 tsvector+BGE-M3+RRF(§7)로 승격. 상세: [0002-following-and-search.md](0002-following-and-search.md).
