...
```ts
interface ArcPoint {                                  // canon §6 / api-contract §3.2 / momentum-engine §7.1
  t: string;                                          // ISO 시각
  value: number;                                      // attention 값 (서버 산출)
  marker?: 1 | 2 | 3 | 4 | 5;                         // 이벤트 마커 — 대부분 없음, 최대 5개만 보유
  eventId?: string;                                   // marker 존재 시 동반 → timeline[].node와 공유
}
...
interface CurrentView {
  ...
  arc: ArcPoint[];                                    // FLAT 시계열 (jsonb) — 대부분 점은 marker 없음
...
**구현:** 단일 연속 시계열은 **모든 `arc` 점**을 시간순(`t`)으로 그대로 사용한다(평탄화 불필요 — 이미 flat `ArcPoint[]`). `area().curve(curveMonotoneX)`로 면, `line()`로 상단 스트로크(current colorKey hue). 번호 마커는 `arc` 중 `marker`가 존재하는 (≤5개) 점만 골라(`arc.filter(p => p.marker != null)`) 그 위치(해당 점의 `t`·`value`)에 번호 원으로 표시한다.

```tsx
const x = scaleTime().domain(extent(arc, p => new Date(p.t))).range([pad, W - pad]);
const y = scaleLinear().domain([0, max(arc, p => p.value)]).range([H - pad, pad]);
const areaGen = area<ArcPoint>()
  .x(p => x(new Date(p.t))).y0(H - pad).y1(p => y(p.value)).curve(curveMonotoneX);
const lineGen = line<ArcPoint>()
  .x(p => x(new Date(p.t))).y(p => y(p.value)).curve(curveMonotoneX);
// 면 + 상단 스트로크: 전체 연속 시계열을 모든 점으로 1회 렌더
// <path d={areaGen(arc)} fill={hex} fillOpacity={0.18} />
// <path d={lineGen(arc)} fill="none" stroke={hex} strokeWidth={2} />

// 번호 마커: marker가 있는 점만 (canon §6, 1..5)
const markers = arc.filter((p): p is Required<ArcPoint> => p.marker != null);
```

```tsx
// 마커: 탭타깃 ≥44px (canon §8). 시각 원은 작게, 히트영역은 44px 투명 원.
<g role="listitem" aria-label={`Moment ${p.marker}: ${dateLabel}`}>
  <circle cx={x(new Date(p.t))} cy={y(p.value)} r={22} fill="transparent"   // 44px hit area
          tabIndex={0} onClick={() => scrollToNode(p.marker)} />
  <circle cx={x(new Date(p.t))} cy={y(p.value)} r={10} fill={hex} stroke="var(--bg)" />
  <text x={x(new Date(p.t))} y={y(p.value)} dy="0.35em" textAnchor="middle"
        fontSize={11} fill="var(--bg)">{p.marker}</text>
</g>
```
...
| AttentionArc | `GET /v1/currents/{id}` | `current_view.arc[]` (flat `ArcPoint`: `{t, value, marker?, eventId?}`) |
