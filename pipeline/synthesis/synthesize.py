...
Model: SYNTHESIS_MODEL (claude-sonnet-4-6 by default, per CANON §9; override via env).
"""
...
def synthesize_brief(current_name: str, docs: list[dict]) -> dict | None:
    ...
    content.append(
        {
            ...
            "text": (
                ...
                f"두 문단을 써라 — 첫 문단: '무엇이 일어나고 있는가'(2~3문장), 둘째 문단: '왜 중요한가'(1~2문장). "
    ...
    try:
        resp = client.messages.create(
            model=SYNTHESIS_MODEL, max_tokens=1024, messages=[{"role": "user", "content": content}]
        )
    except Exception:
        return None
    ...
    return {
        "whatsHappening": paras[0] if paras else full,
        "whyItMatters": " ".join(paras[1:]) if len(paras) > 1 else "",
        "citations": citations,
    }
